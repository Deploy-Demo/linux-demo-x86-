...
bool SegPostprocess::RunXV2(const std::vector<DataBlob>& outputs,
  ...
  //LOGC("Info", "In RunXV2() label_map_size:[%d x %d]", outputs[0].shape[1],outputs[0].shape[2]);

  std::vector<int> score_map_shape = outputs[1].shape;
  //LOGC("Info", "score_map_shape:%d", outputs[1].shape);
  //LOGC("Info", "outputs[0].dtype:%d", outputs[0].dtype);
  int score_map_size =
